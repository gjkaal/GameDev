...
Color Graphics::PeekPixel(int x, int y)
{
	assert(x >= 0);
	assert(x < int(Graphics::ScreenWidth));
	assert(y >= 0);
...
void Graphics::DrawHLine(int x, int y, int length, Color c)
{
	assert(x >= 0);
	assert(x < int(Graphics::ScreenWidth));
	assert(y >= 0);
	assert(y < int(Graphics::ScreenHeight));
	assert(x + length >= 0);
	assert(x + length < int(Graphics::ScreenWidth));
	for (int n = 0; n < length; n++) {
		pSysBuffer[Graphics::ScreenWidth * y + x + n] = c;
	}
}

void Graphics::DrawVLine(int x, int y, int length, Color c)
{
	assert(x >= 0);
	assert(x < int(Graphics::ScreenWidth));
	assert(y >= 0);
	assert(y < int(Graphics::ScreenHeight));
	assert(y + length >= 0);
	assert(y + length < int(Graphics::ScreenWidth));
	
	for (int n = 0; n < length; n++) {		
		pSysBuffer[Graphics::ScreenWidth * (y + n) + x] = c;
	}
}
...
void Graphics::DrawRect(int x1, int y1, int x2, int y2, Color c)
{
	...
	assert(x2 >= x1);
	assert(x2 < int(Graphics::ScreenWidth));
	assert(y2 >= y1);
	assert(y2 < int(Graphics::ScreenHeight));
	int xd; int yd;
	xd = x2 - x1;
	yd = y2 - y1;
	...
	// four lines
	for (int x = x1; x <= x2; x ++) {
		pSysBuffer[Graphics::ScreenWidth * y1  + x] = c;
		pSysBuffer[Graphics::ScreenWidth * (y1 + yd) + x] = c;
	}

	for (int y = y1+1; y < y2; y ++) {
		pSysBuffer[Graphics::ScreenWidth * y + x1] = c;
		pSysBuffer[Graphics::ScreenWidth * y + x1 + xd] = c;
	}
}
